 ...
 * </p>
 * <ul>
 * <li>Apenas agendamentos com status CONCLUIDO são considerados no
 * faturamento</li>
...
public class FinanceiroService {
     ...
     * 
     * <p>
     * Cache habilitado: relatórios são cacheados por barbearia e período para
     * melhor performance.
     * </p>
     * 
     ...
     * @return Ticket médio arredondado para 2 casas decimais, ou 0 se não houver
     *         agendamentos
     */
    ...
    @Transactional
    public JpaReceita adicionarReceita(Long barbeariaId, ReceitaExtraRequestDto dto) {
        if (barbeariaId == null)
            throw new IllegalArgumentException("ID da barbearia obrigatório");
        if (dto == null)
            throw new IllegalArgumentException("Dados da receita obrigatórios");

        JpaReceita receita = new JpaReceita();
        receita.setBarbeariaId(barbeariaId);
        receita.setValor(dto.valor());
        receita.setCategoria(dto.categoria());
        receita.setDescricao(dto.descricao());
        receita.setDataTransacao(dto.dataTransacao());

        return receitaRepository.save(receita);
    }

    @Transactional
    public JpaReceita editarReceita(Long barbeariaId, Long receitaId, ReceitaExtraRequestDto dto) {
        java.util.Objects.requireNonNull(receitaId, "ID da receita não pode ser nulo");
        JpaReceita receita = receitaRepository.findById(receitaId)
                .orElseThrow(() -> new IllegalArgumentException("Receita não encontrada"));

        if (!receita.getBarbeariaId().equals(barbeariaId)) {
    ...
    @Transactional
    public void removerReceita(Long barbeariaId, Long receitaId) {
        java.util.Objects.requireNonNull(receitaId, "ID da receita não pode ser nulo");
        JpaReceita receita = receitaRepository.findById(receitaId)
                .orElseThrow(() -> new IllegalArgumentException("Receita não encontrada"));
    ...
    public List<JpaReceita> listarReceitas(Long barbeariaId, LocalDate inicio, LocalDate fim) {
        if (inicio != null && fim != null) {
            return receitaRepository.findByBarbeariaIdAndDataTransacaoBetweenOrderByDataTransacaoDesc(
                    barbeariaId, inicio, fim);
        }
        return receitaRepository.findByBarbeariaIdOrderByDataTransacaoDesc(barbeariaId);
    }

    // ==================== Gestão de Despesas ====================

    @Transactional
    public JpaDespesa adicionarDespesa(Long barbeariaId, DespesaRequestDto dto) {
        if (barbeariaId == null)
            throw new IllegalArgumentException("ID da barbearia obrigatório");
        if (dto == null)
            throw new IllegalArgumentException("Dados da despesa obrigatórios");

        JpaDespesa despesa = new JpaDespesa();
        despesa.setBarbeariaId(barbeariaId);
        despesa.setValor(dto.valor());
        despesa.setCategoria(dto.categoria());
        despesa.setDescricao(dto.descricao());
        despesa.setDataTransacao(dto.dataTransacao());

        return despesaRepository.save(despesa);
    }

    @Transactional
    public JpaDespesa editarDespesa(Long barbeariaId, Long despesaId, DespesaRequestDto dto) {
        java.util.Objects.requireNonNull(despesaId, "ID da despesa não pode ser nulo");
        JpaDespesa despesa = despesaRepository.findById(despesaId)
                .orElseThrow(() -> new IllegalArgumentException("Despesa não encontrada"));

        if (!despesa.getBarbeariaId().equals(barbeariaId)) {
    ...
    @Transactional
    public void removerDespesa(Long barbeariaId, Long despesaId) {
        java.util.Objects.requireNonNull(despesaId, "ID da despesa não pode ser nulo");
        JpaDespesa despesa = despesaRepository.findById(despesaId)
                .orElseThrow(() -> new IllegalArgumentException("Despesa não encontrada"));
    ...
    public List<JpaDespesa> listarDespesas(Long barbeariaId, LocalDate inicio, LocalDate fim) {
        if (inicio != null && fim != null) {
            return despesaRepository.findByBarbeariaIdAndDataTransacaoBetweenOrderByDataTransacaoDesc(
                    barbeariaId, inicio, fim);
        }
        return despesaRepository.findByBarbeariaIdOrderByDataTransacaoDesc(barbeariaId);
    }
}
