...
public interface FuncionarioRepository extends JpaRepository<JpaFuncionario, Long> {
        ...
        /**
         * Encontra funcionários por barbearia e tipo de perfil (ativos).
         */
        @Query("SELECT f FROM JpaFuncionario f WHERE f.barbeariaId = :barbeariaId AND f.perfilType = :perfilType AND f.ativo = true")
        List<JpaFuncionario> findByBarbeariaIdAndPerfilTypeAtivo(
                        @Param("barbeariaId") Long barbeariaId,
                        @Param("perfilType") com.barbearia.domain.enums.TipoPerfil perfilType);
}
